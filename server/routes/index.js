...
router.patch('/updateHolding', validator.addHolding, holdingsController.updateHolding, (req, res) => {
	return res.status(200).send(res.locals.holdings);
});

// delete updated qtys (DB)
router.delete('/deleteHolding', validator.deleteHolding, holdingsController.deleteHolding, (req, res) => {
	return res.status(200).send(res.locals.holdings);
});

// get current closing price for all ticker symbols (API)
router.get('/closingPrice/:id', updateStocksController.getTickers, updateStocksController.getClosingPrice, (req, res) => {
	return res.status(200).send(res.locals.holdings);
});
