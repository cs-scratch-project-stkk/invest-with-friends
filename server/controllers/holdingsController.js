...
holdingsController.addHolding = async (req, res, next) => {
    try {
        ...
        res.locals.addHoldingSuccess = stock_id 
            ? await holdingsService.addHoldingExisting(user_id, stock_id, ticker, shares) 
            : await holdingsService.addHoldingNew(user_id, ticker, shares);
        res.locals.holdings = await holdingsService.getHoldings(user_id);
        return next();
    } catch(err) {
        return next(err)
